...
export default function AdminDashboard() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccess(`Patient ${form.full_name} created. Share: ${form.email} / ${form.password}`)
    ...
    <div className="space-y-8 animate-fade-in">
      <section className="bg-white rounded-2xl border border-slate-200/80 shadow-card overflow-hidden">
        <div className="px-6 py-5 border-b border-slate-100 bg-slate-50/50">
          ...
          <p className="text-slate-500 text-sm mt-1">Create a patient user before discharge. Share login credentials.</p>
        </div>
        <div className="p-6">
          {error && <div className="mb-6 p-4 rounded-xl bg-red-50 border border-red-100 text-red-700 text-sm">{error}</div>}
          {success && <div className="mb-6 p-4 rounded-xl bg-primary-50 border border-primary-100 text-primary-800 text-sm">{success}</div>}
          <form onSubmit={handleSubmit} className="space-y-4 max-w-lg">
            ...
            <div><label className="block text-sm font-medium text-slate-700 mb-1.5">Email</label><input type="email" value={form.email} onChange={(e) => setForm((f) => ({ ...f, email: e.target.value }))} className={inputClass} required /></div>
            <div><label className="block text-sm font-medium text-slate-700 mb-1.5">Password</label><input type="text" value={form.password} onChange={(e) => setForm((f) => ({ ...f, password: e.target.value }))} className={inputClass} placeholder="Share with patient" required /></div>
            ...
            <button type="submit" disabled={loading} className="px-5 py-3 rounded-xl bg-primary-600 text-white font-semibold hover:bg-primary-700 disabled:opacity-50">{loading ? 'Creating...' : 'Create Patient'}</button>
          </form>
        </div>
      </section>
      <section className="bg-white rounded-2xl border border-slate-200/80 shadow-card overflow-hidden">
          ...
          {patients.length === 0 ? <p className="text-slate-500">No patients yet.</p> : (
            <div className="overflow-x-auto rounded-xl border border-slate-200">
              <table className="min-w-full text-sm">
                <thead><tr className="bg-slate-50 border-b border-slate-200"><th className="text-left py-3 px-4 font-semibold text-slate-700">Name</th><th className="text-left py-3 px-4 font-semibold text-slate-700">Email</th><th className="text-left py-3 px-4 font-semibold text-slate-700">Phone</th></tr></thead>
                <tbody>
                  {patients.map((p) => (
                    <tr key={p.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50/50"><td className="py-3 px-4 font-medium">{p.full_name}</td><td className="py-3 px-4 text-slate-600">{p.email}</td><td className="py-3 px-4 text-slate-600">{p.phone || '—'}</td></tr>
                  ))}
                </tbody>
              </table>
            </div>
